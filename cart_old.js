export default function prepareCart() {
    ...
    function addToCart(event) {
        if (!document.getElementById(`el-${event.target.parentNode.getAttribute('id')}`)) {
            addStruct(event.target.parentNode.getAttribute('id'),
                event.target.parentNode.parentNode.childNodes[3].innerHTML,
                event.target.parentNode.parentNode.childNodes[7].innerHTML);
        } else {
            modifyStruct(event.target.parentNode.getAttribute('id'));
        }
    }
